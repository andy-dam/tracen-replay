...
- `analyzer/` is the Python analyzer: the `tracen_replay` package, its `pyproject.toml`, `tests/`, `tools/` (snapshotting the analyzer's code, and building the dataset for, training and judging the learned reader) and `lab/` (learning the recognizer's confusions, and the test that needs the installed OCR models). Install it with `pip install -e ./analyzer[vision]` and run the suite from the repository root with `python -X utf8 -m unittest discover -s analyzer/tests -t analyzer`; it needs nothing local. The test that needs the installed OCR models lives in `analyzer/lab/tests` and runs the same way with `-s analyzer/lab/tests`.
