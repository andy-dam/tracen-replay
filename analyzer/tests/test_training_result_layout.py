...
class TrainingResultLayoutTests(unittest.TestCase):
    ...
    @unittest.skipUnless(ACTUAL_RAW.is_file(), "fourth result source cache is unavailable")
    def test_actual_fourth_detector_lines_are_recognized(self):
        raw = json.loads(ACTUAL_RAW.read_text(encoding="utf-8"))
        result = detect_training_result_layout(
            raw["lines"],
            header=raw["header"],
            current_grid=raw["current_grid"],
            result_grid=raw["result_grid"],
        )

        self.assertTrue(result["result_grid"])
        self.assertEqual(result["status"], "recognized")
        self.assertGreaterEqual(result["observed"]["paired_card_count"], 2)

    @unittest.skipUnless(SHOGI_RAW.is_file(), "Shogi result source cache is unavailable")
    def test_shogi_committed_cards_are_recognized_without_outcome_banner(self):
        raw = json.loads(SHOGI_RAW.read_text(encoding="utf-8"))
        result = detect_training_result_layout(
            raw["lines"],
            header=raw["header"],
            current_grid=raw["current_grid"],
        ...
        self.assertTrue(result["result_grid"])
        self.assertEqual(result["layout_kind"], "committed_training_result_cards")
        self.assertEqual(result["outcome_status"], "banner_not_visible")
        self.assertGreaterEqual(result["observed"]["paired_card_count"], 4)

    @unittest.skipUnless(CURRENT_MENU_RAW.is_file(), "current-menu source cache is unavailable")
    def test_read_training_current_menu_does_not_become_result_or_preview(self):
        """The high-rate reader must keep a current stat menu out of result phase.

        Use the detector lines from a real current-menu source frame and a
        ``Training`` header to exercise the source-shaped transition.  The
        fixed high-rate crops are deliberately blank, so any positive result
        must come from the general detector's geometry rather than fabricated
        crop values.
        """
        from unittest.mock import patch

        raw_source = json.loads(CURRENT_MENU_RAW.read_text(encoding="utf-8"))
        detector_lines = copy.deepcopy(raw_source["lines"])
        for entry in detector_lines:
            if str(entry.get("text", "")).strip().casefold() == "career":
                entry["text"] = "Training"
                break
        boxes = [
            np.asarray([
        ...
        reader.models = {}
        reader.fingerprint = "current-menu-result-negative"
        ...
        with patch("tracen_replay.preview_recovery.recover_in_memory",
            ...
            result = reader.read_training(pane)

        self.assertFalse(result["result_grid"])
        self.assertNotIn("training_result_layout", result)
        self.assertNotIn("preview_panel", result)
