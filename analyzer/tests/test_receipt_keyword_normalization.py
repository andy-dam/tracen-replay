...
class ReceiptKeywordNormalizationTests(unittest.TestCase):
    def setUp(self):
        self.raw = json.loads(Path('analyzer/tests/fixtures/receipt-keyword-249000.json').read_text(encoding='utf-8'))
    ...
    def test_source_friendship_verb_preserves_recipient_and_amount(self):
        raw = json.loads(Path('analyzer/tests/fixtures/receipt-keyword-499000.json').read_text(encoding='utf-8'))
