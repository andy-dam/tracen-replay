...
class ReceiptGrammarTests(unittest.TestCase):
    def test_source_cursor_over_fixed_word_preserves_recipient_and_amount(self):
        fixture = json.loads(Path('analyzer/tests/fixtures/receipt-fixed-grammar-484750.json').read_text(encoding='utf-8'))
    ...
    def test_actual_click_particle_frames_do_not_create_partial_names(self):
        fixtures=json.loads(Path('analyzer/tests/fixtures/receipt-boundary-obstructions.json').read_text(encoding='utf-8'))
