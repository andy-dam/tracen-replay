...
def main():
    ...
    for name, _, _ in SECTIONS:
        plan.append(['scripts/audit_baseline_sources.py',r,'--shard',name,'--output',f'{r}/{name}/integrity-audit.json'])
    ...
    plan += [[f'{r}/grade_middle_a.py'], [f'{r}/grade_tail_a.py'],
             ...
             ['scripts/audit_baseline_reference_density.py',r,'--output',f'{r}/reference-density.json'],
             ['scripts/audit_baseline_occurrences.py',r,'--output',f'{r}/occurrence-audit.json'],
             ['scripts/build_baseline_scorecard.py',r,'--output',f'{r}/combined-scorecard.json','--markdown','docs/full-run-baseline-scorecard.md']]
    records = []
