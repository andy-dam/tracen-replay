...
def selected_files(root):
    ...
    files = []
    for folder in ('tracen_replay', 'scripts'):
...
def capture(root, output):
    ...
    if any(output.is_relative_to(root / name) for name in ('scripts', 'tracen_replay')):
    ...
    manifest = dict(schema_version='tracen-replay/implementation-snapshot-v1',
                    ...
                    scope='Analyzer package, evaluator/scripts, package resources and project metadata; no acceptance claim.')
