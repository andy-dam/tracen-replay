...
def _source_screen(raw: dict[str, Any]) -> str | None:
    ...
    try:
        ...
        return None
    return screen if isinstance(screen, str) else None
...
def _price_reading(text: Any, confidence: Any, *, crop_box: list[int] | None, source_box: list[int] | None,
                   crop_hash: str,
                   geometry_hash: str, forced_reason: str | None = None) -> dict[str, Any]:
    ...
    return dict(text=normalized, confidence=score, value=value,
                ...
                geometry_sha256=geometry_hash, coordinate_space="gameplay_pane")
...
def build(pane: Image.Image, raw: dict[str, Any], evidence_sha256: str, reader: Any) -> dict[str, Any]:
    ...
    images: list[Any] = []
    image_metadata: list[tuple[int, int, list[int], list[int], str]] = []
    import numpy as np
    ...
    for card in cards:
        for field_index, (source_box, crop_box) in enumerate(
            ...
            image_metadata.append((card["card_index"], field_index, source_box, crop_box, crop_hash))
    ...
    for metadata, (text, confidence) in zip(image_metadata, readings):
        card_index, field_index, source_box, crop_box, crop_hash = metadata
        by_card[card_index].append(
            _price_reading(text, confidence, crop_box=crop_box, source_box=source_box,
                           crop_hash=crop_hash, geometry_hash=cards[card_index]["geometry_sha256"])
        )
    for card in cards:
...
def _validate_price(price: Any, expected_field: str, expected_source: list[int] | None,
    ...
    parsed = _price_reading(text, confidence, crop_box=expected_crop, source_box=expected_source,
                            crop_hash=crop_hash, geometry_hash=geometry_hash)
...
def observe(pane: Image.Image, raw: dict[str, Any], extra: dict[str, Any]) -> dict[str, Any]:
    ...
    for candidate, stored in zip(candidates, extra["cards"]):
        ...
        result_cards.append(dict(
            ...
            status="complete" if not reasons else "unknown",
...
def apply(row: dict[str, Any], raw: dict[str, Any], extra: dict[str, Any], evidence_path: str | Path) -> dict[str, Any]:
...
def generate(root: str | Path, *, start_ms: int, end_ms: int, frame_ids: list[str] | None = None,
    ...
    for path, raw in selected:
        ...
        with Image.open(evidence) as image:
            extra = build(image.convert("RGB"), raw, file_fingerprint(evidence), reader)
