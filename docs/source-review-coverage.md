...
Combined declared sampled-effect coverage is now 365 seconds, or 20.544% of the 1776.717-second recording, including the continuous opening 280 seconds. The next uncovered source begins at 4:40; the whole-recording queue schedules larger blocks from there. The untouched initial evaluation remains unchanged; new reviews are development evidence. Coverage includes failing references and is not a passing-coverage percentage.

The 2:40–4:40 reference adds 480 source-selected 250 ms samples, 24 receipt groups and nine completed training actions. It separately records browsing and the blocked race attempt. The initial effect score and subsequent source-based boundary corrections are retained separately from analyzer-fix scores; see [measured results](results-gameplay-only.md). Dialogue choices without visible selection evidence remain unresolved.
...
Reference labels also need observability checks. The recipient suffix in the 262250–263000 ms receipt is cursor-covered in every reviewed target frame; a complete spelling in a different receipt cannot establish that target identity. A separate annotated reference version records that uncertainty while preserving the expected spelling and all 58 effects. Its result remains reference agreement, not a claim that every expected field is observable. Original labels and scores remain available.

Effect references may attach `observability_exceptions` with a group index, effect index, affected fields, explanation, and reviewed timestamps inside that group. Evaluation carries these annotations into its output without changing the expected count, matching rules, missing effects, or pass/fail result. An absent annotation means observability has not been adjudicated; it does not certify that every label is readable. This keeps a hidden field visible as a validation limitation without rewarding a guessed value or silently improving recall by removing it.
