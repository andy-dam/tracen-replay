...
Reference labels also need observability checks. The recipient suffix in the 262250–263000 ms receipt is cursor-covered in every reviewed target frame; a complete spelling in a different receipt cannot establish that target identity. The preserved reference's expected spelling therefore needs an explicit uncertainty annotation. Until that adjudication is reflected in scoring, its result is reference agreement, not a claim that every expected field is observable. Preserve original labels and scores when correcting this distinction.
