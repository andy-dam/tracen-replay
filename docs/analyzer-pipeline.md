...
| --- | --- | --- | --- |
| `training_gain_recovery` | a training result has conflicting gain digits, a source-bound candidate that was not accepted, a gain read on exactly one result frame that the event did not accept or filled from the state instead, a committed result with no signed gain read at all, a chosen training whose result no sampled frame caught (reason `training_without_result_frames`: the 1.5 s before the next screen recognized within 10 s after it, where a skipped card shows), a card seen only as a candidate on a frame or two with no gain read (the player skipped through it), or a committed result whose stat badges were accepted while a performance row stayed unread (`performance_rows_unread`; reason `performance_row_unread_on_committed_result`, the accepted badges bind each reread frame to the card); every reread of a result covers it from half a second before its first sampled frame, where the badges animate in, to half a second after its last, for every gain not yet read; a card left on screen longer than a window's 1.5 s span is reread from its first frames, where the badges appear, rather than not at all; a result with no gain read at all is also reread at the card's own earlier moment when an unclassified frame up to 2.5 s before its frames shows two of the stat names along the card's row (reason `card_shown_before_result_frames`: the card came and went in a quarter second, and the event was dated by the empty transition after it) | 60 fps | up to 96 windows and 120 s of source |
...
under the confidence a receipt parse needs, or under an overlay the frame
recorded over it (`stationary_stat_receipt_across_covered_line`; the later
read may be the line the covered-line reread restored), or, for exactly one
frame, nothing at all in the receipt band, the award's own animation having
covered the box, with the line back on the same pixels after it. The chain
of samples is the proof, however long the line stays. The later outcome
...
letting the accounting work the amount out from a state difference; when
such a result's frames are only the empty transition after the card (the
card itself came and went in a quarter second, caught once mid animation
with two of its stat names reading through under the banner), the card's
own earlier moment is reread too. A digit
...
between the confirmation and the scene do not count against it. When no
