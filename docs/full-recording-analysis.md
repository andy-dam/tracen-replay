...
The causal accounting never invents a receipt from a state difference, with one flagged exception recorded with basis `turn_difference`: inside one turn window a field's remaining difference goes to its only possible owner. The owners are the turn's sole training whose result was not read at all (or whose reading of that field conflicted; a panel read without a row for the field did not raise it), the one outcome whose receipt named the field but lost its number (`Power went up by.`, the number cut, covered or misread; the digits still visible and the up/down direction must agree with the difference), for a negative performance difference the one lesson or song bought in the window whose cost was not observed, and for skill points in a race turn the one race, when no training with unread skill points could have paid them. A training whose panel was read can still own a field in two flagged ways: a clipped badge, where the read digits are the leading digits of the read value plus the difference (a 1 that was 11, a 3 that was 35; the entry shows the completed amount and keeps the read digits), and a visible candidate, where a result frame showed the difference (or its leading digits) for a field that a conflicting frame dropped. Two possible owners or a caption with no owning event leave the difference open with its time window. The assignment is stored on the owner (`turn_difference_gains`, `turn_difference_performance_gains`, `turn_difference_captions`, `turn_difference_cost`, `turn_difference_completions`, `turn_difference_basis`) and as a derived contribution, and every consumer shows it as worked out from the difference between turns so a viewer can replace it.

Receipt captions and purchases: a story caption read whole and read with its tail cut off, or with one or two misread letters inside a long caption, is one outcome (`_same_caption`), so a race reward panel is not counted once per frame; an animation banner beside a preview-only training whose preview window overlaps the banner is that training, not a second one. A zero balance is drawn dim and reads with low confidence: a lone single-digit `0` in a currency slot of the lesson menu, the confirmation dialog's leftover row, or an offer card's cost row is zero (`dim_zero_currency_fields`), and any other low-confidence card price is unknown for its field rather than a reason to discard the card. A receipt whose every spelling is a bounded OCR variant of the one name the request dialog showed is that item (`confirmed_request_name_with_bounded_ocr_variants`; an extra word or a digit is another item); a song title read both with its proven note glyph and as a stray letter keeps only the proven spelling; a song receipt whose fixed phrase was misread (`Learnd te ong "Title".`) is repaired by bounded edit distance against `Learned the song`, the quoted title being the anchor (`fixed_phrase_repair`), and a `Learned` line carrying a quoted title is never a named acquisition. A blank Learn-press frame does not end a request run, a currency label bleeding into the dialog's title box is not a second name, the repeated balance before and after a purchase is the nearest pair of complete equal frames, a leftover value the dialog shows on two or more frames wins over a value it showed once (the fading frame after Learn), and a counter hidden by the cursor leaves that field to the card's printed price. A purchase priced from repeated balances counts as an observed debit in the accounting, like one whose projection the next balance matched, and the debit is dated at the window from its request to the first frame showing the debited balance (`debit_window_ms`), where the points actually left, rather than at the receipt; the repeated balance frame after that confirms the debit but does not extend it past the balance comparison it belongs to.

Result banners and re-reads: a result word missing its final glyph (`SUCCES`, `FAILUR`) is that word when the same frame proves the result panel, with a clipped failure taken before a clipped success; a failed training's displayed performance awards are recorded as `unawarded_performance_projection`, never applied. A gain read on exactly one result frame that the event did not accept requests a bounded 60 fps re-read of the result interval (`single_frame_training_gain`), so a fresh run observes the badge instead of the accounting working it out. Partial song names are priced by repeated balances like any other purchase.
