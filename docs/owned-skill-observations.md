...
## Partial panels and the third recording

Panel recognition now checks the card layout as well as the shared tabs and final attributes. The Inspiration view shares that surrounding interface, so its Legacy Origin section is a negative layout signal. Skill rows must form a consistent two-column grid, with wrapped text kept within its card. A visible unique-skill level can anchor a partial panel. A single final card is accepted only at the expected edge of an established grid; an isolated interior heading does not become a skill. Names containing digits or equals signs remain eligible.

The third recording's cached-source reconstruction retains all 13 reviewed visible base names, including the unpaired final card, and verifies the visible U=ma2 level of 6. The Pace Chaser Savvy circle variant remains unknown: the existing pixel detector does not provide sufficient consistent evidence. This result is development validation after source review, not a fresh independent score.

The same integration preserves the first two recordings' 14 reviewed names each and their previously verified level and circle details. All non-inventory gameplay collections match the preceding reports exactly. The full suite passed 877 tests at this integration checkpoint. These checks concern the visible panels; optional screen visits, complete scrolling, and pauses are not prerequisites for analyzing a recording. Missing views and unreadable details remain explicitly unavailable.
