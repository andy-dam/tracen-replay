...
class InventoryPanelTests(unittest.TestCase):
    def setUp(self):
        self.raws=[x['raw'] for x in json.loads(Path('tests/fixtures/final-owned-cards.json').read_text())['frames']]
        self.extras=json.loads(Path('tests/fixtures/final-owned-panel.json').read_text())
