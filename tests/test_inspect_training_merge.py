...
class ActualT042MergeTests(unittest.TestCase):
    ...
    INSPECTION = ROOT / 'training-inspection.json'
    _available = all(path.is_file() for path in (RAW, REFINEMENT, SOURCE_FRAME, INSPECTION))

    @unittest.skipUnless(_available, 'frozen T042 prepared evidence is not present')
    def test_t042_partial_inspection_keeps_speed_wit_and_state_observation(self):
        root = self.ROOT
        raw = json.loads(self.RAW.read_text(encoding='utf-8'))
        refined = load_numeric_refinement(
            raw,
            self.REFINEMENT,
            original=raw,
        ...
        base.update(
            ...
            numeric_cap_refinement=refined['numeric_cap_refinement'],
        )
        inspection = json.loads(self.INSPECTION.read_text(encoding='utf-8'))
        supplemental = next(
            item for item in inspection['readings']
            if item['source_timestamp_ms'] == raw['source_timestamp_ms']
        )

        got = merge([base], [supplemental])[0]
        values = got['facts']['result_values']
        caps = got['facts']['stat_caps']

        self.assertEqual(values['speed'], 599)
        self.assertEqual(values['wit'], 576)
        self.assertEqual(caps['speed'], 1600)
        self.assertEqual(caps['wit'], 1300)
        self.assertEqual(
            got['facts']['inspection_merge_provenance']['result_values']['speed']['origin'],
            'base',
        )
        observations = build_observations([got])
        stats = next(item for item in observations if item['payload']['channel'] == 'stats')
        self.assertEqual(stats['payload']['values']['speed'], 599)
        self.assertEqual(stats['payload']['values']['wit'], 576)
        self.assertEqual(stats['payload']['caps']['speed'], 1600)
        self.assertEqual(stats['payload']['caps']['wit'], 1300)
